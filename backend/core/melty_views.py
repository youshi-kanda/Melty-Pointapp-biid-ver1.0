...
from .melty_integration import melty_sso, melty_user_service, MeltyIntegrationError
...
def melty_auth_url(request):
    ...
    try:
        ...
        auth_url = melty_sso.generate_auth_url(state=state)
...
def melty_callback(request):
    ...
    try:
        ...
        # melty SSO処理
        try:
            user, access_token = melty_sso.handle_callback(code, state)
...
def register_with_melty(request):
    ...
    try:
        ...
        if not all([melty_code, email, first_name, last_name]):
            return Response({
                'success': False,
                'error': '必須項目が不足しています'
