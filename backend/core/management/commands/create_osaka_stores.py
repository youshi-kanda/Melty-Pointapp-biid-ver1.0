...
class Command(BaseCommand):
    help = '大阪エリア（ミナミ・北新地）の初期店舗データを作成'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS("大阪エリア店舗データ作成コマンド"))
        self.stdout.write("=" * 60)
        self.stdout.write("")
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f"✅ サービスエリア取得成功"))
            self.stdout.write(f"   - {minami_area.area_name} ({minami_area.area_code})")
            self.stdout.write(f"   - {kitashinchi_area.area_name} ({kitashinchi_area.area_code})")
            self.stdout.write("")
        except ServiceArea.DoesNotExist:
            self.stdout.write(self.style.ERROR("❌ エラー: サービスエリアが見つかりません"))
            self.stdout.write("   マイグレーション0004_add_service_areas.pyを実行してください")
            return

        # ミナミエリアの店舗作成
        self.stdout.write("📍 ミナミエリアの店舗作成中...")
        minami_count = 0
        for store_data in MINAMI_STORES:
            ...
            if created:
                ...
                self.stdout.write(f"   - {store.name} (既存)")

        self.stdout.write(self.style.SUCCESS(f"✅ ミナミエリア: {minami_count}件の新規店舗を作成"))
        self.stdout.write("")

        # 北新地エリアの店舗作成
        self.stdout.write("📍 北新地エリアの店舗作成中...")
        kitashinchi_count = 0
        ...
        self.stdout.write(self.style.SUCCESS(f"✅ 北新地エリア: {kitashinchi_count}件の新規店舗を作成"))
        self.stdout.write("")
        ...
        self.stdout.write("=" * 60)
        self.stdout.write(f"総店舗数: {total_stores}件")
        self.stdout.write(f"  - ミナミエリア: {Store.objects.filter(service_area=minami_area).count()}件")
        self.stdout.write(f"  - 北新地エリア: {Store.objects.filter(service_area=kitashinchi_area).count()}件")
        self.stdout.write("")

        # カテゴリ別統計
        self.stdout.write("カテゴリ別:")
        ...
        for category, count in category_counts.items():
            self.stdout.write(f"  - {category}: {count}件")
        self.stdout.write("")
